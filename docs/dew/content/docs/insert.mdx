---
title: INSERT
description: Insert rows with the Inserter builder.
---

The `Inserter` builder constructs INSERT queries. Create one via `Table.Insert(db)` or `dew.Insert[T](db, schema)`.

## Insert with values

```go
err := Users.Insert(db).
    Columns(Users.Name, Users.Email).
    Values("Alice", "alice@example.com").
    Exec()
```

## Multiple rows

```go
err := Users.Insert(db).
    Columns(Users.Name, Users.Email).
    Values("Alice", "alice@example.com").
    Values("Bob", "bob@example.com").
    Exec()
```

## Insert with models

Pass struct pointers directly — columns are inferred from struct fields:

```go
user := &User{Name: "Alice", Email: "alice@example.com"}

err := Users.Insert(db).
    Models(user).
    Exec()
```

Multiple models:

```go
err := Users.Insert(db).
    Models(user1, user2, user3).
    Exec()
```

<Callout type="warn">
Do not use `Columns()` with `Models()` — columns are inferred from the struct. Mixing them returns an error.
</Callout>

## Returning

PostgreSQL `RETURNING` clause:

```go
sql, args, _ := Users.Insert(db).
    Columns(Users.Name).
    Values("Alice").
    Returning(Users.ID).
    ToSql()
// INSERT INTO users (name) VALUES ($1) RETURNING users.id
```

### ScanWith + Returning

```go
inserted, err := Users.Insert(db).
    Columns(Users.Name).
    Values("Alice").
    Returning(Users.ID, Users.Name).
    ScanWith(func(rows *sql.Rows) (*User, error) {
        var u User
        err := rows.Scan(&u.ID, &u.Name)
        return &u, err
    })
```

<Callout type="info">
If your struct implements `RowScanner`, prefer that over `ScanWith`. See [RowScanner](/docs/select#rowscanner-interface).
</Callout>

## On conflict (upsert)

`OnConflict()` starts a conflict handler. Specify the conflict target columns, then choose an action.

### Do nothing

Skip rows that conflict:

```go
err := Users.Insert(db).
    Columns(Users.Name, Users.Email).
    Values("Alice", "alice@example.com").
    OnConflict(Users.Email).
    DoNothing().
    Exec()
// INSERT INTO users (name, email) VALUES ($1, $2)
// ON CONFLICT (email) DO NOTHING
```

### Do update

Update specific columns on conflict:

```go
err := Users.Insert(db).
    Columns(Users.Name, Users.Email).
    Values("Alice", "alice@example.com").
    OnConflict(Users.Email).
    SetUpdate(Users.Name, "Alice Updated").
    Exec()
// INSERT INTO users (name, email) VALUES ($1, $2)
// ON CONFLICT (email) DO UPDATE SET name = $3
```

### Update with EXCLUDED

Use `dew.Raw` to reference the `EXCLUDED` row (the row that failed to insert):

```go
err := Users.Insert(db).
    Columns(Users.Name, Users.Email).
    Values("Alice", "alice@example.com").
    OnConflict(Users.Email).
    SetUpdate(Users.Name, dew.Raw("EXCLUDED.name")).
    Exec()
// INSERT INTO users (name, email) VALUES ($1, $2)
// ON CONFLICT (email) DO UPDATE SET name = EXCLUDED.name
```

### Multiple conflict targets

```go
err := Users.Insert(db).
    Models(user).
    OnConflict(Users.Email, Users.TenantID).
    DoNothing().
    Exec()
// ON CONFLICT (email, tenant_id) DO NOTHING
```

### Multiple SET updates

Chain `SetUpdate` calls:

```go
err := Users.Insert(db).
    Models(user).
    OnConflict(Users.Email).
    SetUpdate(Users.Name, dew.Raw("EXCLUDED.name")).
    SetUpdate(Users.Age, dew.Raw("EXCLUDED.age")).
    Exec()
// ON CONFLICT (email) DO UPDATE SET name = EXCLUDED.name, age = EXCLUDED.age
```

### Upsert with RETURNING

```go
result, err := Users.Insert(db).
    Columns(Users.Name, Users.Email).
    Values("Alice", "alice@example.com").
    OnConflict(Users.Email).
    SetUpdate(Users.Name, dew.Raw("EXCLUDED.name")).
    Returning(Users.ID, Users.Name).
    ScanWith(func(rows *sql.Rows) (*User, error) {
        var u User
        err := rows.Scan(&u.ID, &u.Name)
        return &u, err
    })
```

### Upsert with batch

```go
err := Users.Insert(db).
    Models(users...).
    OnConflict(Users.Email).
    DoNothing().
    Batch(1000).
    Exec()
// Splits into chunked INSERT ... ON CONFLICT statements
```

## Batch insert

`Batch(size)` splits large inserts into multiple statements, avoiding database parameter limits (e.g. PostgreSQL's 65535 limit):

```go
err := Users.Insert(db).
    Models(users...).
    Batch(1000).
    Exec()
// Executes multiple INSERT statements, each with up to 1000 rows
```

Works with values too:

```go
err := Users.Insert(db).
    Columns(Users.Name, Users.Email).
    Values("Alice", "alice@example.com").
    Values("Bob", "bob@example.com").
    Values("Carol", "carol@example.com").
    Batch(2).
    Exec()
// 2 statements: first with Alice+Bob, second with Carol
```

### Batch with upsert

```go
err := Users.Insert(db).
    Models(users...).
    OnConflict(Users.Email).
    DoNothing().
    Batch(1000).
    Exec()
```

### Inspect batch queries

`BatchQueries()` returns the SQL and args for each chunk without executing:

```go
queries, args, err := Users.Insert(db).
    Models(users...).
    Batch(1000).
    BatchQueries()
// queries[0] = "INSERT INTO users (...) VALUES ($1, ...), ($5, ...)"
// args[0]    = [...]
```

## ToSql

```go
sql, args, err := Users.Insert(db).
    Columns(Users.Name).
    Values("Alice").
    ToSql()
```

## Context

```go
err := Users.Insert(db).
    Columns(Users.Name).
    Values("Alice").
    Exec(ctx)
```
