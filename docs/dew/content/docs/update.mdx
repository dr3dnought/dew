---
title: UPDATE
description: Update rows with the Updator builder.
---

The `Updator` builder constructs UPDATE queries. Create one via `Table.Update(db)` or `dew.Update[T](db, schema)`.
