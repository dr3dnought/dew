	...
	tests := []struct {
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "SELECT * FROM users WHERE id = $1",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "INSERT INTO users (name, age) VALUES ($1, $2)",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "UPDATE users SET name = $6 WHERE id = $7",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "SELECT 'Is this a question?'",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "SELECT 'Question?' FROM table WHERE id = $1 AND text = 'Really?'",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "SELECT 'It''s a match?' WHERE id = $1",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "SELECT 'O''Reilly' WHERE id = $1",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "SELECT 'Привет?' WHERE name = $1",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "INSERT INTO t VALUES ($1, $2, $3, $4, $5)",
		...
		},
		{
			...
			want: map[string]string{
				...
				"DuckDB":     "SELECT 'test' WHERE id = $1 AND name = 'test?'",
